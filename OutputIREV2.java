...
Identical to OutputIREV.java, except that instead of specifying a simulator
settings file, you specify a directory containing one or more such files.  
This allows simulations with different settings to be run sequentially.

Run from the Windows Command Prompt 
"java OutputIREV absolutefilepathtodirectorycontainingfiles", 

The files must have an identical format to /parafiles/initial.txt.  
...

{
	...
	* 6. Whether or not masses of entities will be varied. 
	* 7. Whether or not the size of the entities will be varied. 
	* 8. Whether or not the preferred speed of the entities will be varied.  
	* 9. Whether or not to produce entity trajectory output. 
	* 10. Whether or not to produce destination occupancy output. 
	* 11. Whether or not to produce entity destination sequences. 
	* 12. Whether or not to produce entity destination distances. 
	* 13. Whether or not to produce entity entity schedules. 
	* 14. The initial x-coordinates of all entities. 
	* 15. The initial y-coordinates of all entities. 
	* 16. The initial masses of the entities. 
	* 17. The initial sizes of the entities. 
	* 18. The initial preferred speeds of the entities. 
	...
	*/
	public OutputIREV2(int Ntemp, int steps, int equilT, int numAct, int sims, 
	...
	public void actionPerformed() {
		...
		/*
		 * Assign starting positions and individual characteristics, if this is
		 * required.
		 */
		...
		// If entity trajectories are desired...
		if (trajOutput) {
			//...state the filename...
			...
			timesteps, the number of destinations, the number of entities, and the
...
//////////////////////////////////////////////// here perform simulation:
		...
		*/
		while (step < steps) {
			...
			// Update the movement of every entity.
		...
		if (occOutput) {
			try {
				...
				// If connection fails to close, give error trace.
				e.printStackTrace();
			}
		}
	}
...
/* Main instance - describes code that is immediately run when the file is run.*/
	...
	public static void main(String[] args) {
		...
		sequences outputted, destination distances outputted, entity schedules 
		outputted.*/
		...
		/* Initial positions, masses, sizes, and speeds of entities, and the 
		arry of model parameters.*/
		...
		try {
			// If an input directory is provided...
			if (args.length > 0) {
			...
			// Otherwise, if a directory is not specified...
				...
				//...then set the default input directory. CHANGE THIS LINE FOR YOUR DIRECTORY STRUCTURE!
				inDir = "Sim/parafiles/";
			}
			
			// Find the input directory.
			File dir = new File(inDir);
			// Filter for filenames inside this directory.
			FilenameFilter fileNameFilter = new FilenameFilter() {

				@Override
				// Find files with specific parts in the filename.
				public boolean accept(File dir, String name) {
					// Check this line for the correct room name.
					if(name.contains("horseshoe_new_dest_nent")) { 
						return true;
					}
					return false;
				}
			};
			// Find all files that match the filter.
			File[] inFiles = dir.listFiles(fileNameFilter);
			
			// If there are no files present...
			if (inFiles.length == 0) {
				//...end this program.
				System.exit(0);
			}
			// Otherwise, if there are files present...
			else {
				//...for each file...
				for (File v : inFiles) {
					//...convert the filename to a string.
					String inFilename = v.toString();
					...
					if (readParas) {
						...
						// Next line gives number of entities present.
						...
						/*
						Next line describes the values of the parameters for the full model.
						*/
						...
						*/
						...
						tmptmp = Integer.valueOf(br.readLine()).intValue();
						xIn = new double[Nin]; // Initial x-positions of each entity
						yIn = new double[Nin]; // Initial y-position of each entity
						massIn = new double[Nin]; // Masses of each entity
						sizeIn = new double[Nin]; // Sizes of each entity
						speedIn = new double[Nin]; // Preferred speeds of each entity
				...
				// place all outputs for all simulation replicates.
					...
					// Otherwise, if a settings file is not provided...
						...
						// Default output directory here. CHANGE THIS LINE FOR YOUR DIRECTORY STRUCTURE!
						outDir = "Sim";
					}
					...
					+ "/" + pieces[0] + "_occ=" + params[0] + "_dist="
					+ params[1] + "_des=" + params[2];
					// Find the input directory.
					...
					if (dire.exists()) {
						...
						// Filter for filenames inside this directory.
						FilenameFilter fileNamFilt = new FilenameFilter() {

							@Override
							// Find files with specific parts in the filename.
							public boolean accept(File dir, String name) {
								// If there are already output from some simulations for this 
								// set of conditions...
							if (name.contains("seq")) {
								//...then return true.
								return true;
							}
							// Otherwise, if this directory does not have any simulation
							// replicate output data.
							return false;
							}
						};
						// Find the number of simulation replicates that have already won.
						File[] finFiles = dire.listFiles(fileNamFilt);
						// Let the user know how many replicates have run.
						System.out.println(finFiles.length + " simulations have already run.");
						// If this set of simulator conditions has finished (assumes that there is up to 100 replicates)...
						if (finFiles.length == 100) { //...simulation replicates hard-coded.
							//...then proceed to the next simulation setting directory.
							continue;
						}
						// If this set of simulation settings don't have any replicates...
						else if (finFiles.length == 0) {
							//...then the replicate index is started at one.
							startSim = 1;
						}
						// Otherwise, if there is greater than one but less than 100...
						else {
							//...then the starting replicate index is the number of completed 
							// replicates.
							startSim = finFiles.length;
						}
					}
					...
					for (int s = startSim-1; s < endSim; s++) {
						...
						new OutputIREV2(Nin, steps, equilSteps, numActs, s, roomtmp, 
